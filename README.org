...
**** write a parser (not going to do in elisp since may want non-emacs users)
...
:END:
      [[file:scratch.ex::###%20Parsing%20date%20regex][last state of regexp]]
...
***** Parse and cast the dates
      ...
      :END:
***** Need to use ecto.multi to make a clean transaction
***** Or maybe need to do versioning right now?
**** Parse logbook and properties to get version and id into object
**** regenerate file from database
**** versioning
- the [[file:lib/ttl/things/things.ex::def%20create_or_update_object(attrs)%20do][create or update function]] needs to be changed 
- need to select all the existing uuid's, match the versions
- and then do an update if it makes sense
*** display UI for agenda
*** additional properties for scheduling + contexts + tags
*** notifications
...
#+CAPTION: Clock summary at [2017-08-15 Tue 13:59]
| Headline                                        | Time    |       |       |      |
|-------------------------------------------------+---------+-------+-------+------|
| *Total time*                                    | *10:19* |       |       |      |
|-------------------------------------------------+---------+-------+-------+------|
